...
- 访客发送后显示“正在思考”
- 可通过 OpenRouter 启用默认 LLM 回复，后台可按会话切换 `LLM / 人工` 回复模式
- 后台回复在访客端流式输出
...
本地开发可以在 `.env` 里接入 OpenRouter，让访客消息默认由 LLM 回复，同时后台管理员仍然可以随时手动回复、停止生成或点击“摊牌”来接管。后台也可以对单个会话切换 `LLM / 人工` 模式，切换后从下一条访客消息开始生效。
