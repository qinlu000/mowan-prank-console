...
本地开发可以在 `.env` 里接入 OpenRouter，让访客消息默认由 LLM 回复，同时后台管理员仍然可以随时手动回复、停止生成或点击“摊牌”来接管。后台也可以对单个会话切换 `LLM / 人工` 模式；人工模式下会调用 LLM 生成参考稿，但只展示给管理员，管理员确认或改写后再发给访客。每个访客问题最终只会保留一条魔丸回复，重新生成会替换原回复。
