...
- 访客首次进入可输入代号，浏览器会自动记住；同一代号最多保留 3 个对话
- 访客发送后显示“正在思考”
- 可通过 OpenRouter 启用默认 LLM 回复，后台可按会话切换 `LLM / 人工` 回复模式；人工模式会生成仅管理员可见的 LLM 参考稿
- 后台回复在访客端流式输出
...
访客第一次打开页面时会输入一个代号。这个代号会保存在浏览器 `localStorage`，后端还会用浏览器 cookie 把当前浏览器绑定到这个代号；同一代号最多创建 3 个对话。
...
本地开发可以在 `.env` 里接入 OpenRouter，让访客消息默认由 LLM 回复，同时后台管理员仍然可以手动回复、停止生成或点击“摊牌”。后台也可以对单个会话切换 `LLM / 人工` 模式，模式切换只影响下一条新访客消息；如果当前这条正在由 LLM 处理，需要点击“接管当前”才会改成人工处理。人工模式下会调用 LLM 生成参考稿，但只展示给管理员，管理员确认或改写后再发给访客。每个访客问题最终只会保留一条魔丸回复，重新生成会替换原回复。
...
- 服务启动时会加载未过期会话
- 服务启动后会定时清理过期会话和消息
- 访客代号不是密码，知道同一个代号的人可以打开同一个会话
- 同一浏览器一旦绑定代号，页面上不能再切换到其他代号；清浏览器数据或换设备仍可能绕过
- 同一代号最多创建 3 个对话，超过后后端会拒绝
- 启用访客代号后，没有代号的旧随机会话会在服务启动时清理掉
- 管理员登录会话、回复记录和摊牌操作会写入 SQLite，便于必要时排查
