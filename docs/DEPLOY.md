...
访客代号会和会话一起保存在 SQLite 中。浏览器会把代号记在 `localStorage`，后端还会用浏览器 cookie 把当前浏览器绑定到这个代号；同一代号最多创建 3 个对话。这个代号不是密码，不适合承载敏感信息。

同一浏览器一旦绑定代号，页面上不能再切换到其他代号；清浏览器数据或换设备仍可能绕过。要强约束身份，需要后续增加暗号或正式登录。

启用访客代号后，没有代号的旧随机会话会在服务启动时清理掉，避免后台混入无法识别身份的老记录。
